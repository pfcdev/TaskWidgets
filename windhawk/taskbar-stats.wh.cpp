...
BOOL Wh_ModInit() {
    Wh_Log(L"TaskbarStats Milestone 1 init");
    ...
    HMODULE user32 = LoadLibraryEx(L"user32.dll", nullptr,
                                   LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (user32) {
        auto createWindowInBand = reinterpret_cast<CreateWindowInBand_t>(
        ...
        auto createWindowInBandEx = reinterpret_cast<CreateWindowInBandEx_t>(
    ...
    if (kernelBase) {
        auto loadLibraryExW =
            reinterpret_cast<LoadLibraryExW_t>(GetProcAddress(kernelBase,
                                                              "LoadLibraryExW"));
        if (loadLibraryExW) {
