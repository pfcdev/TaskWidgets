...
MediaStatus ReadPreviousStatus(const std::wstring& appDir) {
    MediaStatus status;
    std::string json = ReadTextFile(JoinPath(appDir, L"media-status.json"));
    if (json.empty()) {
        return status;
    }
...
void WriteStatus(const std::wstring& appDir, const MediaStatus& status) {
    std::wstring path = JoinPath(appDir, L"media-status.json");
    ...
    json += "  \"updatedAtUnix\": " + std::to_string(CurrentUnixTime());
