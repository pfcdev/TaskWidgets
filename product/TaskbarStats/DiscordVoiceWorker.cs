...
internal static class DiscordVoiceWorker
{
    ...
    private static readonly string StatusPath = Path.Combine(AppDirectory, "discord-voice-status.json");
    private static readonly string SettingsPath = Path.Combine(AppDirectory, "widget-settings.json");
    ...
    private static DiscordSettings ReadSettings()
    {
        try
        {
            ...
            return new DiscordSettings
            {
                Enabled = root.TryGetProperty("discordEnabled", out var enabled) &&
                          enabled.ValueKind == JsonValueKind.True,
                ClientId = GetString(root, "discordClientId"),
                ClientSecret = GetString(root, "discordClientSecret"),
                RedirectUri = GetString(root, "discordRedirectUri")
            };
        }
        catch (Exception ex)
        {
